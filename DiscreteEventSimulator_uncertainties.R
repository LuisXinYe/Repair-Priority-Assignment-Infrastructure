...
allResult <- parSapply(cl, 1:NumScenarios, function(j){
  ...
  simulation_time <- 240*24
  
  simulate_poisson_process <- function(lambda, max_time) {
  ...
  event_times <- vector("list", length = NumBaseStation)
  lambda_estimate <- vector("numeric", length = NumBaseStation)
  for (i in 1:NumBaseStation) {
    event_times[[i]] <- simulate_poisson_process(lambda[i], max_time = simulation_time)
    
    lambda_estimate[i] <- length(event_times[[i]]) / simulation_time
  }
        ...
        if (length(failed_set) == 0) {
          ...
          if (state[1] != 0) {
            ...
            if (index_policy == 1) { # cmu-lambda-rule
              next_completion_time <- rexp(1, mu[which.max(cost*mu/lambda)])
              next_event_time <- next_completion_time
              state[1] <- 0
            ...
            if (index_policy == 2) { # cmu-rule
              next_completion_time <- rexp(1, mu[which.max(cost*mu)])
              next_event_time <- next_completion_time
              state[1] <- 0
            ...
            if (index_policy == 4) { # nearest-first
              next_completion_time <- rexp(1, mu[which.min(manhattan_distance)])
              next_event_time <- next_completion_time
              state[1] <- 0
              state[which.min(manhattan_distance)+1] <- 0
              fail_time[which.min(manhattan_distance)] <- ObservationWindow
              total_cost <- total_cost + sum(cost/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
              time <- time + next_event_time # update time
            }
            if (index_policy == 5) { # mu-priority
            ...
            if (index_policy == 6) { # heuristic
              if (which.max(cost*mu/lambda_estimate) == which.max(cost*mu)) {
                next_completion_time <- rexp(1, mu[which.max(cost*mu/lambda_estimate)])
                next_event_time <- next_completion_time
                state[1] <- 0
                state[which.max(cost*mu)+1] <- 0
                total_cost <- total_cost + sum(cost/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                ...
                if (mu[which.max(cost*mu)] >= mu[which.max(cost*mu/lambda_estimate)]) {
                  next_completion_time <- rexp(1, mu[which.max(cost*mu/lambda_estimate)])
                  next_event_time <- next_completion_time
                  state[1] <- 0
                  state[which.max(cost*mu/lambda_estimate)+1] <- 0
                  total_cost <- total_cost + sum(cost/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                  time <- time + next_event_time # update time
                }
                else {
                  if ((cost*mu)[which.max(cost*mu/lambda_estimate)]>(1-(mu[which.max(cost*mu)]-mu[which.max(cost*mu/lambda_estimate)])/(Lambda+alpha))*max(cost*mu)) {
                    next_completion_time <- rexp(1, mu[which.max(cost*mu/lambda_estimate)])
                    next_event_time <- next_completion_time
                    state[1] <- 0
                    state[which.max(cost*mu/lambda_estimate)+1] <- 0
          ...
          if (state[1] != 0) {
            ...
            if (index_policy == 1) { # cmu-lambda-rule
              next_completion_time <- rexp(1, mu[failed_set[which.max(cost[failed_set]*mu[failed_set]/lambda_estimate[failed_set])]])
              if (next_completion_time < min(next_failure_time)) { # completion
                next_event_time <- next_completion_time
                total_cost <- total_cost + sum(cost*state[-1]/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                state[failed_set[which.max(cost[failed_set]*mu[failed_set]/lambda_estimate[failed_set])]+1] <- 0
                time <- time + next_event_time # update time}
              }
              else { # an new failure arrival
                next_event_time <- min(next_failure_time)
                total_cost <- total_cost + sum(cost*state[-1]/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                state[1] <- failed_set[which.max(cost[failed_set]*mu[failed_set]/lambda_estimate[failed_set])]
                state[normal_set[which.min(next_failure_time)]+1] <- 1
                time <- time + next_event_time # update time}
              }
            }
            ...
            if (index_policy == 4) { # nearest-first
              next_completion_time <- rexp(1, mu[failed_set[which.min(manhattan_distance[failed_set])]])
              if (next_completion_time < min(next_failure_time)) { # completion
                next_event_time <- next_completion_time
                total_cost <- total_cost + sum(cost*state[-1]/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                state[failed_set[which.min(manhattan_distance[failed_set])]+1] <- 0
                time <- time + next_event_time # update time}
              }
              else { # an new failure arrival
                next_event_time <- min(next_failure_time)
                ...
                state[1] <- failed_set[which.min(manhattan_distance[failed_set])]
                time <- time + next_event_time # update time}
              }
            }
            if (index_policy == 5) { # mu-priority
            ...
            if (index_policy == 6) { # heuristic
              if (which.max((cost*mu/lambda_estimate)[failed_set]) == which.max((cost*mu)[failed_set])) {
                ...
                if (mu[failed_set[which.max((cost*mu)[failed_set])]] >= mu[failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]]) {
                  next_completion_time <- rexp(1, mu[failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]])
                  if (next_completion_time < min(next_failure_time)) { # completion
                    next_event_time <- next_completion_time
                    total_cost <- total_cost + sum(cost*state[-1]/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                    state[failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]+1] <- 0
                    time <- time + next_event_time # update time}
                  }
                  else { # an new failure arrival
                    next_event_time <- min(next_failure_time)
                    total_cost <- total_cost + sum(cost*state[-1]/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
                    state[1] <- failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]
                    state[normal_set[which.min(next_failure_time)]+1] <- 1
                    time <- time + next_event_time # update time}
                  }
                }
                else {
                  if ((cost*mu)[failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]]>(1-(mu[failed_set[which.max((cost*mu)[failed_set])]]-mu[failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]])/(Lambda+alpha))*max((cost*mu)[failed_set])) {
                    next_completion_time <- rexp(1, mu[failed_set[which.max((cost*mu/lambda_estimate)[failed_set])]])
                    if (next_completion_time < min(next_failure_time)) { # completion
                      next_event_time <- next_completion_time
                      total_cost <- total_cost + sum(cost*state[-1]/alpha*(exp(-alpha*time)-exp(-alpha*(time+next_event_time))))
...
save(list = c("allResult"), file = paste0("NumBaseStation", NumBaseStation, "uncertainties", ".Rdata"))
